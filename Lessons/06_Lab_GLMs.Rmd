...
Problem2 <- ggplot(EPAair, aes(Ozone, fill = as.factor(Year))) +
  geom_density(alpha = 0.5) +
  labs( title = "Density plot: Ozone by year", x = "Ozone AQI", y = "Density", fill = "Year") +
  scale_fill_manual(values= c('blue','yellow'))
print(Problem2)


Problem3 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
   stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5, label = c("a", "b"), color = 'red') +
  labs( x = "", y = "Ozone AQI values")
print(Problem3)

Problem4 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_violin() +
   stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5, label = c("a", "b"), color = 'red') +
  labs( x = "", y = "Ozone AQI values")
print(Problem4)
...
the_lm6 <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + meanLat + meanLng)
summary(the_lm6)
