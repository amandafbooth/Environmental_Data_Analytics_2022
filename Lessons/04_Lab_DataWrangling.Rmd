...
NTL.1999 <- filter(NTL.phys.data, year4 %in% c(1999))


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.tues.1990s <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake"), year4 %in% c(1990:1999))
...
NTL.tues.1990s.july <- NTL.phys.data %>%
...
NTL.irradiance <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance_type", values_to = "Irradiance")


#Exercise 6: spread temperatureC into more than one column based on the depth

reverse_pivot <- pivot_wider(NTL.irradiance, names_from = Irradiance_type, values_from = Irradiance)
